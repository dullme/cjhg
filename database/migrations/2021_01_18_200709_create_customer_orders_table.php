...
class CreateCustomerOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('customer_orders', function (Blueprint $table) {
            ...
            $table->decimal('pick_up',15, 5)->comment('提货运费');
            $table->decimal('transportation',15, 5)->comment('运输运费');
            $table->decimal('deliver_goods',15, 5)->comment('送货运费');
            $table->decimal('freight',15, 5)->default(0)->comment('总运费');
            $table->string('logistics')->comment('物流公司');
            $table->decimal('vat',15, 5)->comment('增值税');
            $table->decimal('amount',15, 5)->default(0)->comment('订单总金额');
            $table->decimal('invoice',15, 5)->default(0)->comment('待开票金额');
            $table->decimal('invoiced',15, 5)->default(0)->comment('已开票金额');
